...
use File;
class LegalofficeAppointmentController extends Controller
{
    ...
    public function todayAppointment(){
        ...
        $schedules=Schedule::where(['lawyer_id'=>$legals->id,'day_id'=>$day->id])->get();
        $today=Date('Y-m-d');
        $appointments=Appointment::where(['lawyer_id'=>Auth::guard('legal')->user()->id,'already_treated'=>0,'date'=>$today,'payment_status'=>1])->get();
        $website_lang=ManageText::all();


        $valid_lang=ValidationText::all();
        $schedule_error=$valid_lang->where('lang_key','req_schedule')->first()->custom_lang;
        return view('legaloffice.appointment.today',compact('appointments','schedules','legals','website_lang','schedule_error'));
    }
    public function newAppointment(){
        $legals=Auth::guard('legal')->user();
        $schedules=Schedule::where('lawyer_id',$legals->id)->get();
        $appointments=Appointment::where(['lawyer_id'=>Auth::guard('legal')->user()->id,'already_treated'=>0,'payment_status'=>1])->get();
        $today=0;
        $website_lang=ManageText::all();
        ...
        return view('legaloffice.appointment.new',compact('appointments','schedules','legals','today','website_lang','to_date_error','from_date_error'));
    }

    public function allAppointment(){
        $appointments=Appointment::where(['lawyer_id'=>Auth::guard('legal')->user()->id])->orderBy('id','desc')->get();
        $website_lang=ManageText::all();
        return view('legaloffice.appointment.all',compact('appointments','website_lang'));
    }
    ...
    public function meet($id){
        ...
        if($appointment){
            if($appointment->lawyer_id==$legals->id){
                if($appointment->already_treated==0){
                    $today=Date('Y-m-d');
                    if($today==$appointment->date){
                        $setting=Setting::first();
                        $website_lang=ManageText::all();
                        return view('legaloffice.appointment.prescription',compact('appointment','setting','legals','website_lang'));
                    }else{
                    ...
                    return redirect()->route('legaloffice.all.appointment')->with($notification);
                }
            }else{
                $notify_lang=NotificationText::all();
                $notification=$notify_lang->where('lang_key','wrong')->first()->custom_lang;
    ...
    public function meetingStore(Request $request,$id){
        ...
        return redirect()->route('legaloffice.already-meet',$id)->with($notification);
    }
    ...
    public function showPrescription($id){
        ...
        return view('legaloffice.show_prescription', compact('doctor', 'appointment', 'setting'));
    }
    ...
    public function editPresciption($id){
        ...
        $legals=Auth::guard('legal')->user();
        $website_lang=ManageText::all();
        return view('legaloffice.appointment.edit-prescription',compact('appointment','setting','legals','website_lang'));
    }
        ...
        if($appointment){
            if($appointment->lawyer_id==$legals->id){
                if($appointment->already_treated==1){
                    $setting=Setting::first();
                    $website_lang=ManageText::all();
                    return view('legaloffice.appointment.meeted',compact('appointment','setting','legals','website_lang'));
                }else{
                    $notify_lang=NotificationText::all();
                    $notification=$notify_lang->where('lang_key','wrong')->first()->custom_lang;
                    $notification=array('messege'=>$notification,'alert-type'=>'error');
    ...
    public function searchAppointment(Request $request){
        ...
        $website_lang=ManageText::all();
        return view('legalloffice.appointment.ajax-appointment',compact('appointments','website_lang'));
    ...
    public function searchParticulerAppointment(Request $request){
        ...
        $website_lang=ManageText::all();
        return view('legalloffice.appointment.ajax-particular-appointment',compact('appointments','website_lang'));
    }
    ...
    public function paymentHistory(){
        ...
        return view('legaloffice.payment.index',compact('appointments','payment','appointment','legals','currency','website_lang','from_date_error'));
    ...
    public function searchPaymentHistory(Request $request){
        ...
        $currency=Setting::first();
        $website_lang=ManageText::all();
        return view('legaloffice.payment.ajax-payment',compact('appointments','payment','appointment','legals','currency','website_lang'));
    }
