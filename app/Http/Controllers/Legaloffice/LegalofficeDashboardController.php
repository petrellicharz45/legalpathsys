...
use App\Setting;
use Carbon\Carbon;
class LegalofficeDashboardController extends Controller
{
    ...
    public function index(){
        ...
        $successAppointments=$appointments->where('already_treated',1);
         ...
         for($i=1; $i<=$length+1; $i++){
            ...
            $sum=Appointment::whereDate('created_at',$date)->where(['payment_status'=>1,'lawyer_id'=>$legals->id])->sum('appointment_fee');
            $data[] = $sum;
         }
         // end manage chart
        ...
        return view('legaloffice.dashboard',compact('newAppointmentQty','successAppointments','todayAppointments','data','appointments','currency','website_lang'));
    }
}
