...
use App\Traits\LegalZoomMeetingTrait;
...
class LegalofficeMeetingController extends Controller
{
    use LegalZoomMeetingTrait;
    ...
    public function index(){
        ...
        $meetings=ZoomMeeting::where('lawyer_id',$legals->id)->orderBy('start_time','desc')->get();
        $website_lang=ManageText::all();
        return view('legaloffice.zoom.meeting.index',compact('meetings','website_lang'));
    }
    ...
    public function createForm(){
        $legals=Auth::guard('legal')->user();
        $appointments=Appointment::where('lawyer_id',$legals->id)->select('user_id')->groupBy('user_id')->get();
        $patients=$appointments;
        $users=User::whereIn('id',$patients)->get();
        $website_lang=ManageText::all();
        return view('legaloffice.zoom.meeting.create',compact('patients','users','website_lang'));
    }
    ...
    public function store(Request $request)
    {
        ...
        $data['agenda']=$legals->name;
        $data['host_video']=1;
        $data['participant_video']=1;
        $response= $this->create($data);
        if($response['success']){
            $meeting=new ZoomMeeting();
            $meeting->lawyer_id=$legals->id;
            ...
            if($request->client==-1){
                ...
                foreach($users as $user){
                    ...
                    $message=str_replace('{{lawyer_name}}',$legals->name,$message);
            ...
            return redirect()->route('legaloffice.zoom-meetings')->with($notification);
        }else{
            $notify_lang=NotificationText::all();
    ...
    public function editForm($id){
        ...
        if($meeting){
            ...
            if($meeting->lawyer_id){
                ...
                $website_lang=ManageText::all();
                return view('legaloffice.zoom.meeting.edit',compact('meeting','users','website_lang'));
            }else{
                $notify_lang=NotificationText::all();
                $notification=$notify_lang->where('lang_key','wrong')->first()->custom_lang;
                $notification=array('messege'=>$notification,'alert-type'=>'error');
    ...
    public function meetingHistory(){
        $legals=Auth::guard('legal')->user();
        $histories=MeetingHistory::where('lawyer_id',$legals->id)->orderBy('meeting_time','desc')->get();
        $website_lang=ManageText::all();
        return view('legaloffice.zoom.meeting.history',compact('histories','website_lang'));
    }
    ...
    public function upCommingMeeting(){
        ...
        $histories=MeetingHistory::where('lawyer_id',$legals->id)->orderBy('meeting_time','desc')->get();
        $website_lang=ManageText::all();
        return view('legaloffice.zoom.meeting.upcoming',compact('histories','website_lang'));
    }
}
