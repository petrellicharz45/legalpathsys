...
class PaymentController extends Controller
{
    ...
    public function payment(){
        ...
        if($user->ready_for_appointment==1){
            ...
            return view('student.profile.payment',compact('appointments','stripe','currency','banner','navigation','website_lang','paymentSetting','razorpay','setting','flutterwave','user','paystack','instamojo','mollie','paymongo'));
        }else{
            $notify_lang=NotificationText::all();
            $notification=$notify_lang->where('lang_key','fillup_profile')->first()->custom_lang;
            $notification=array('messege'=>$notification,'alert-type'=>'error');
            return redirect()->route('student.account')->with($notification);
        };
    ...
    public function stripePayment(Request $request){
        ...
        return redirect()->route('student.order')->with($notification);
    }
    ...
    public function razorPay(Request $request){
        ...
        if(count($input)  && !empty($input['razorpay_payment_id'])) {
            try {
                ...
                return redirect()->route('student.payment')->with($notification);
            }
        }
    }
    ...
    public function molliePayment(){
        ...
        $payment = Mollie::api()->payments()->create([
            ...
            'redirectUrl' => route('student.mollie-payment-success'),
        ]);
    ...
    public function payWithInstamojo(){
        ...
        $payload = Array(
            ...
            'redirect_url' => route('student.instamojo-response'),
            'send_email' => true,
    ...
    public function payWithPaymongo(Request $request){
        ...
        $student = new \GuzzleHttp\student();
        ...
        try{
            $response = $student->request('POST', 'https://api.paymongo.com/v1/payment_methods', [
        ...
        $response = $student->request('POST', 'https://api.paymongo.com/v1/payment_intents', [
        ...
        $intent_student_key = $intent_response['data']['attributes']['student_key'];
        $intent_id = $intent_response['data']['id'];

        $student = new \GuzzleHttp\student();

        // create payment
        $payment_response = $student->request('POST', 'https://api.paymongo.com/v1/payment_intents/'.$intent_id.'/attach', [
        'body' => '{"data":{"attributes":{"payment_method":"'.$payment_method_id.'","student_key":"'.$intent_student_key.'"}}}',
        'headers' => [
            'Accept' => 'application/json',
            'Authorization' => 'Basic '.$secret_code.'',
            'Content-Type' => 'application/json',
    ...
    public function payWithPaymongoGrabPay(){
        ...
        $price = $total_price * $paymongoPayment->currency_rate;
        $price = round($price);
        $success_url = route('student.paymongo-payment-success');
        $faild_url = route('student.paymongo-payment-cancled');
        $currency_code = $paymongoPayment->currency_code;
        ...
        $code = base64_encode($paymongoPayment->public_key.':'.$paymongoPayment->secret_key);
        $student = new \GuzzleHttp\student();
        $response = $student->request('POST', 'https://api.paymongo.com/v1/sources', [
