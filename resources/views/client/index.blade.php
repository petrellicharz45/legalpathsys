...
@endphp
@if ($lawyer_section->show_homepage==1)
<!--Team Area Start-->
<div class="team-area">
    <div class="container">
        <div class="row">
                ...
                <div class="team-carousel owl-carousel">
                    ...
                    <div class="team-item">
                        ...
                        <div class="team-text">
                            ...
                            <p><span><i class="fas fa-graduation-cap"></i> {{ ucfirst($lawyer->desgination->desgination) }}</span></p>
...
<div class="blog-area bg_ecf1f8">
    <div class="container">
        ...
        <div class="row">
            ...
            @endif
            @php
$probono_section=$homesections->where('section_type',8)->first();
@endphp
@if ($probono_count !=0)
@if ($probono_section->show_homepage==1)
<!--Blog-Area Start-->
<div class="blog-area ">
    <div class="container">
        <div class="row">
            <div class="col-md-25 mt_15">
                <div class="main-headline">
                    <h1><span>{{ ucfirst($probono_section->first_header) }}</span> {{ ucfirst($probono_section->second_header) }}</h1>
                    <p>{{ $probono_section->description }}</p>
                </div>
            </div>
        </div>
        <div class="row">
            <div class="col-md-6">
                <div class="blog-item first-blog">
                    <a href="{{ url($feature_blog->feature_image) }}" class="image-effect">
                        <div class="blog-image">
                            <img src="/uploads/pdf/{{($feature_probono->image) }}" alt="">
                        </div>
                    </a>
                    <div class="blog-text">
                        <div class="blog-author">
                            <span><i class="fas fa-user"></i> {{ $website_lang->where('lang_key','admin')->first()->custom_lang }}</span>
                            <span><i class="far fa-calendar-alt"></i> {{ $feature_probono->created_at->format('m-d-Y') }}</span>
                        </div>
                        <h3><a href="{{ url('probono-details/'.$feature_probono->title) }}">{{ $feature_probono->title }}</a></h3>
                        <p>
                            {{ $feature_probono->description }}
                        </p>

                        @if ($isRtl=='RTL')
                        <a class="sm_btn" href="{{ url('probono-details/'.$feature_probono->title) }}">{{ $website_lang->where('lang_key','see_details')->first()->custom_lang }} ←</a>
                        @else
                            <a class="sm_btn" href="{{ url('probono-details/'.$feature_probono->title) }}">{{ $website_lang->where('lang_key','see_details')->first()->custom_lang }} →</a>
                        @endif
            ...
            <div class="col-md-6">
                <div class="blog-carousel owl-carousel">
                    @php $i=0; @endphp
                    @foreach ($probonos->take($probono_section->content_quantity) as $probonos)
                        @php $i++; @endphp
                        @if($i == 1)
                            @continue
                        @endif
                        <div class="blog-item effect-item">
                            <a href="#" class="image-effect">
                                <div class="blog-image">
                                    <img src="/uploads/pdf/{{($probonos->image) }}" alt="Blog Thumbnail Image">
                                </div>
                            </a>
                            <div class="blog-text">
                                <div class="blog-author">
                                ...
                                @if ($isRtl=='RTL')
                                <a class="sm_btn" href="{{ url('probono-details/'.$probonos->title) }}">{{ $website_lang->where('lang_key','see_details')->first()->custom_lang }} ←</a>
                                @else
                                <a class="sm_btn" href="{{ url('probono-details/'.$probonos->slug) }}">{{ $website_lang->where('lang_key','see_details')->first()->custom_lang }} →</a>
                                @endif
                            </div>
                        </div>
                    @endforeach
